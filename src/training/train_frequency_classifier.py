...
def downsample_majority_class(
    ...
    FF-c23 has fake:real = 6:1, so fake samples are reduced
    to match the number of real samples in the training set.
    """
    ...
    if len(majority_df) == 0 or len(minority_df) == 0:
        raise ValueError(
            f"Both classes must exist for downsampling. "
            f"majority={len(majority_df)}, minority={len(minority_df)}"
        )
    ...
    balanced_df = pd.concat(
        [minority_df, sampled_majority_df],
        axis=0,
    )
...
def evaluate_classifier(model, x, y) -> dict:
    ...
    if proba is not None and len(set(y)) == 2:
        metrics["roc_auc"] = float(roc_auc_score(y, proba))

    return metrics


def train_frequency_model_with_splits(
    train_csv: str,
    val_csv: str,
    test_csv: str | None,
    label_col: str,
    ...
    balanced_train_distribution = train_df[label_col].value_counts().sort_index().to_dict()
    ...
    result = {
        ...
        "train_balancing": {
            ...
            "used_train_distribution": balanced_train_distribution,
        },
